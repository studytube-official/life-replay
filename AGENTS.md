...
- **現行形式(①②)に場所名は無い** → 外部検索はせず、HOME/WORK分類と端末内の手動ラベルだけで補完する設計
...
## 実装済み機能(2026-07-10時点 v0.4)

3形式パーサー / ZIP読込 / 最新データ追加(訪問・移動・経路すべて重複排除) / OS別の公式エクスポート案内 / スタッツ集計(訪問・場所・距離・ストリーク・時間帯/曜日/月別グラフ) / カテゴリ別集計(コンビニ・カフェ等17種) / 実績28個+解除セレモニー演出 / XP・レベル・称号 / Leafletオフライン冒険マップ(速度切替・シーク・追従・軌跡、外部タイルなし) / 場所名編集・カテゴリ変更UI / HOME・WORKの端末内名称補完 / デモモード / ゼロスタートモード / IndexedDB自動復元 / 訪問計測

## 未完了・ロードマップ

1. **analytics.sql のSupabase実行待ち**(オーナー作業) → 実行後 `?stats` の動作確認
2. **オーナーの実データテスト** — 数週間後に実データ(iOS形式)を読み込み、エッジケースと追加更新を確認
3. **デイリーチェックイン機能**(検討済み・未着手) — Geolocation APIで「今日を刻む」ボタン+ストリーク+限定実績の構想。ただし位置は端末内だけで扱う
4. **Phase 2: ネイティブ化**(構想) — Capacitor + iOS Share ExtensionでエクスポートJSONを共有シートから直接受信。将来はVisit Monitoringでアプリ自身が記録。Apple Developer Program($99/年)は未契約
5. 細かい改善、実績の追加など
...
- 位置データを外部送信するコードを絶対に入れないこと。外部地図タイル・逆ジオコーディング・第三者スクリプトも禁止
- `vite.config.js` が本番ビルドへ注入するCSPを緩めない。Supabase送信は固定のvisit文字列だけに保つ
